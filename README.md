...
| `actor_container_retirement_guard = true` | Loading over a live game can retire a dynamically generated NPC or creature base whose container list is already malformed. Vanilla follows and frees the untrusted item/node pointers while destroying the actor. | Validates the complete retiring container against the active allocator and engine object contract. A valid list is unchanged; a corrupt list is detached as one unit before the original destructor continues. This guard works in allocator modes `0`, `1`, and `2`. |
