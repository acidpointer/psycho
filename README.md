...
| `performance.radio_signal_scan_cache` | `true` | Caches only the periodic nearby-radio worker scan; direct Pip-Boy and station-selection calls remain vanilla. | Set `false` if station availability seems stale or another plugin owns the same worker callsite. |
| `performance.radio_signal_scan_cache_ttl_ms` | `2000` | Keeps a periodic radio-scan result for this many milliseconds. Values are clamped to `500..10000`. | Lower it for quicker station refresh; raise it to reduce more repeated TTW worldspace scanning. |
