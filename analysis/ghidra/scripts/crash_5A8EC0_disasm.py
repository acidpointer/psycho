# @category Analysis
# @description Disassemble FUN_005a8ea0 crash site and decompile chain functions.
...
def disasm_func_instructions(addr_int, label):
    """Disassemble all instructions in the function containing addr_int."""
    ...
    write("  Total: %d instructions" % func.getBody().getNumAddresses())


def find_callers_sample(addr_int, label, limit=15):
    write("")
    write("-" * 70)
    ...
    while refs.hasNext():
        ...
        if count >= limit:
            break
    write("  Total: %d refs (showing %d)" % (count, min(count, limit)))


# --- Main ---

write("CRASH DISASSEMBLY: FUN_005a8ea0 at +0x20")
write("=" * 70)

# 1. Disassemble FUN_005a8ea0
disasm_func_instructions(0x005A8EA0, "FUN_005a8ea0")

# 2. Decompile chain functions
...
find_callers_sample(0x006815C0, "FUN_006815c0")

# --- Output ---
outpath = "/data/storage0/Workspace/psycho/analysis/ghidra/output/crash/crash_5A8EC0_disasm.txt"
fout = open(outpath, "w")
fout.write("\n".join(output))
fout.close()
