# @category Analysis
# @description Dump close terrain PPLighting descriptor fields and defines for PBR contract research
...
TARGET_VERTEX_INDICES = [53, 54, 78, 79, 80, 81, 82, 83]
TARGET_PIXEL_INDICES = [60, 61, 80, 81, 82, 83, 84, 85, 86]
	...
	while index < len(records):
		...
		if rel >= 0:
			...
			if shader_index >= 0 and shader_index < group_count and target_contains(target_indices, shader_index):
				...
				write("    field=0x%02X write=0x%08x rhs=%-10s value=%-36s :: %s" % (field, record[0], record[3], format_value(record[4]), record[1]))
				found += 1
		index += 1
	write("")
...
def print_target_disasm_windows():
	addresses = [
		(0x00B73485, "VS close land index 53"),
		(0x00B734B9, "VS close land index 54"),
		(0x00B73A19, "VS close land index 78"),
		(0x00B73AFE, "VS close land index 82"),
		(0x00B75CA0, "PS close land index 60"),
		(0x00B75CC7, "PS close land index 61"),
		(0x00B76196, "PS close land index 80"),
		(0x00B762AE, "PS close land index 85"),
	]
	index = 0
	while index < len(addresses):
		item = addresses[index]
		disasm_window(item[0], 18, 26, item[1])
		index += 1

def main():
	write("FNV PBR CLOSE TERRAIN DESCRIPTOR CONTRACT")
	write("")
	write("Goal: dump the exact fields and define pointers for close land PPLighting records before another terrain replacement.")
	decompile_at(PPLIGHTING_VERTEX_CONSTRUCTOR_ADDR, "PPLighting vertex constructor", 3500)
	decompile_at(PPLIGHTING_PIXEL_CONSTRUCTOR_ADDR, "PPLighting pixel constructor", 3500)
	scan_descriptor_writes(PPLIGHTING_VERTEX_CONSTRUCTOR_ADDR, 0x00B74210, PPLIGHTING_VERTEX_GROUP_C_BASE_LEA_ADDR, "PPLighting vertex group C close land", PPLIGHTING_VERTEX_GROUP_C_COUNT, TARGET_VERTEX_INDICES)
	scan_descriptor_writes(PPLIGHTING_PIXEL_CONSTRUCTOR_ADDR, 0x00B78A00, PPLIGHTING_PIXEL_GROUP_B_BASE_LEA_ADDR, "PPLighting pixel group B close land", PPLIGHTING_PIXEL_GROUP_B_COUNT, TARGET_PIXEL_INDICES)
	print_target_disasm_windows()
	print_watch_refs()
...
outpath = "/data/storage1/Workspace/psycho/analysis/ghidra/output/perf/graphics_fnv_pbr_close_terrain_descriptor_contract.txt"
fout = open(outpath, "w")
fout.write("\n".join(output))
fout.close()
write("")
