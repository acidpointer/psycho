# @category Analysis
# @description Audit FNV PPLighting material texture property fields and texture-set slot sources for native PBR
...
KNOWN = {
	...
	0x0046EBF0: "TextureSet count/capacity helper used by Decal Map count",
	0x00540FE0: "TESLandTexture texture set validator",
	0x005453B0: "model/geometry texture set application",
	0x005922E0: "BGSTextureSet constructor",
	0x00593140: "NullTextureSet singleton setup",
	0x00653270: "attached-data writer/helper",
	...
	0x00B70590: "PPLighting active-object resource iterator 00B70590",
	0x00B70600: "PPLighting active-object resource iterator 00B70600",
	0x00B70680: "PPLighting active-object resource iterator 00B70680",
	0x00B70700: "PPLighting active-object resource iterator 00B70700",
	0x00B707D0: "PPLighting active-object resource iterator 00B707D0",
	...
	0x01033C7C: "BGSTextureSet +0x30 slot-interface vtable candidate",
	0x01033D1C: "BGSTextureSet main vtable candidate",
	0x010AE0D0: "B-range shader-property vtable candidate",
	0x011F444C: "source-texture renderer-data key used by resolver",
	0x011F49A4: "property/key used by extra map enumerator",
	0x011F91E0: "current geometry/proxy global",
	0x011F95EC: "shader/material manager global",
}
...
DECOMPILE_TARGETS = [
	(0x0046E8E0, "extra named texture map attach/name helper", 12000),
	(0x0046E910, "extra named texture map enumerator", 22000),
	(0x0046EB00, "Dark Map getter", 8000),
	(0x0046EB20, "Detail Map getter", 8000),
	(0x0046EB40, "Gloss Map getter", 8000),
	(0x0046EB60, "Glow Map getter", 8000),
	(0x0046EB80, "Bump Map getter", 8000),
	(0x0046EBA0, "Decal Map getter", 8000),
	(0x0046EBD0, "Decal Map count getter", 8000),
	(0x0046EBF0, "TextureSet count/capacity helper used by Decal Map count", 12000),
	(0x00877A30, "TextureSet indexed slot storage getter", 14000),
	(0x00540FE0, "TESLandTexture texture set validator", 26000),
	(0x005453B0, "model/geometry texture set application", 18000),
	(0x005922E0, "BGSTextureSet constructor", 18000),
	(0x00593140, "NullTextureSet singleton setup", 18000),
	(0x00653270, "attached-data writer/helper", 12000),
	(0x00653290, "attached-data/property helper", 12000),
	(0x00A59D30, "NiAVObject property lookup by type", 12000),
	(0x00B55480, "shader/model texture binding candidate", 18000),
	(0x00B66640, "PPLighting texture-array flag initializer", 24000),
	(0x00B68660, "PPLighting six texture-array writer", 36000),
	(0x00B690D0, "PPLighting texture/effect serialization", 52000),
	(0x00BDAF10, "PPLighting material/property helper", 32000),
	(0x00BDB4A0, "PPLighting setup variant before BDF790", 52000),
	(0x00BDF790, "PPLighting selector/pass-entry driver", 70000),
]
...
def scan_disasm_pattern_windows(addr_int, label):
	...
	write("")
	write("=" * 70)
	write("DISASM FIELD/SLOT WINDOWS: %s @ 0x%08x" % (label, addr_int))
	write("=" * 70)
			...
			if index < len(pushes):
				...
				if is_register_text(arg):
					...
					if arg_inst is not None:
						print_register_defs_before(arg_inst, arg.upper(), 42, 3, "      ")
			else:
...
def decompile_vtable_slots(base, label):
	...
	for slot in VTABLE_SLOTS:
		...
		if func is None:
			continue
		if slot in [0x8C, 0x90, 0x94, 0x98, 0x9C, 0xA0, 0xA4, 0xA8, 0xAC, 0xB0, 0xB4, 0xB8, 0xBC, 0xC0, 0xC4, 0xCC, 0xDC, 0x114, 0x128, 0x130, 0x134]:
			decompile_at(ptr, "%s slot +0x%03x" % (label, slot), 12000)
...
def print_direct_call_sections():
	...
	for item in SLOT_CALL_FUNCTIONS:
		print_direct_call_table(item[0], item[1], targets, 8)

def print_calls_from_key_functions():
	for item in DECOMPILE_TARGETS:
		find_and_print_calls_from(item[0], item[1])

def main():
	write("FNV PBR PPLIGHTING TEXTURE PROPERTY FIELD SOURCE AUDIT")
	write("")
	write("Purpose:")
	write("1. Identify the producer-side material texture fields behind PPLighting BA9EE0 resources.")
	write("2. Audit A59D30(3) property fields +0xAC..+0xC0 and BGSTextureSet +0x30 vtable +0x8C texture slots.")
	write("3. Keep native PBR blocked unless diffuse/normal/glow/etc. resource provenance reaches final SetTexture stages.")
	print_key_refs()
	print_vtables()
	print_core_decompiles()
	print_pattern_sections()
	print_direct_call_sections()
	print_calls_from_key_functions()

main()

outpath = "/data/storage1/Workspace/psycho/analysis/ghidra/output/perf/graphics_fnv_pbr_pplighting_texture_property_field_source_audit.txt"
fout = open(outpath, "w")
fout.write("\n".join(output))
fout.close()
write("")
