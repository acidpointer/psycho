...
type MenuModeFn = unsafe extern "C" fn() -> bool;
...
pub(super) unsafe fn capture(input_owner: *mut c_void) -> Option<NativeInputState> {
    ...
    let menu_mode = unsafe { menu_mode_function()() };
...
fn menu_mode_function() -> MenuModeFn {
    // xNVSE independently uses this exact cdecl contract in its gameplay hook.
    unsafe { core::mem::transmute(0x0070_2360usize) }
}
