...
| LandLOD | `TerrainLODTemplate.hlsl` | LandLOD vertex shader, `BaseMap s0`, `NormalMap s1`, parent maps `s4/s6`, noise `s7`, `LODTexParams c31`, `LandLODSpec c38`, NVR terrain include `c89/c90` | Medium | OMV base LandLOD now uses the terrain register split and is gated on the VPT contract. Projected-shadow LandLOD and terrain fade remain separate unproven ABIs. |
