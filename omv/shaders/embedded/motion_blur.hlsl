// OMV camera motion blur.
//
// The current depth and the current/previous camera transforms reconstruct a
// screen-space exposure path. Samples run from the current pixel toward its
// previous-frame position: this is a trailing shutter integration, not a
// symmetric smear that invents a leading trail.

#ifndef MOTION_BLUR_SAMPLES
#define MOTION_BLUR_SAMPLES 7
#endif

sampler2D SceneColor : register(s0);
sampler2D WorldDepth : register(s1);
sampler2D FirstPersonDepth : register(s2);

float4 ScreenData : register(c0);
float4 MotionOptions : register(c1);
float4 LayerFlags : register(c2);
float4 CurrentWorldFrustum : register(c3);
float4 CurrentWorldDepth : register(c4);
float4 WorldRow0 : register(c5);
float4 WorldRow1 : register(c6);
float4 WorldRow2 : register(c7);
float4 PreviousWorldFrustum : register(c8);
float4 PreviousWorldDepth : register(c9);
float4 CurrentFirstPersonFrustum : register(c10);
float4 CurrentFirstPersonDepth : register(c11);
float4 FirstPersonRow0 : register(c12);
float4 FirstPersonRow1 : register(c13);
float4 FirstPersonRow2 : register(c14);
float4 PreviousFirstPersonFrustum : register(c15);
float4 PreviousFirstPersonDepth : register(c16);

static const float DepthEndpointEpsilon = 0.000001f;

float Smooth01(float value) {
    value = saturate(value);
    return value * value * (3.0f - 2.0f * value);
}

float2 DepthTexelCenter(float2 uv) {
    float2 pixel = clamp(floor(uv * ScreenData.xy), 0.0f, ScreenData.xy - 1.0f);
    return (pixel + 0.5f) * ScreenData.zw;
}

bool GeometryDepth(float rawDepth) {
    return rawDepth > DepthEndpointEpsilon
        && rawDepth < (1.0f - DepthEndpointEpsilon);
}

bool SkyDepth(float rawDepth, bool reversedDepth) {
    return reversedDepth
        ? rawDepth <= DepthEndpointEpsilon
        : rawDepth >= (1.0f - DepthEndpointEpsilon);
}

float LinearDepth(float rawDepth, float2 nearFar, bool reversedDepth) {
    float nearZ = max(nearFar.x, 0.01f);
    float farZ = max(nearFar.y, nearZ + 1.0f);
    return reversedDepth
        ? nearZ * farZ / max(rawDepth * (farZ - nearZ) + nearZ, 0.001f)
        : nearZ * farZ / max(farZ - rawDepth * (farZ - nearZ), 0.001f);
}

float3 ViewRay(float2 uv, float4 frustum) {
    return float3(
        lerp(frustum.x, frustum.y, uv.x),
        lerp(frustum.w, frustum.z, uv.y),
        1.0f
    );
}

float2 ProjectPrevious(float3 position, float4 frustum) {
    float2 view = position.xy / max(position.z, 0.001f);
    return float2(
        (view.x - frustum.x) / max(frustum.y - frustum.x, 0.001f),
        (frustum.w - view.y) / max(frustum.w - frustum.z, 0.001f)
    );
}

float3 TransformPrevious(float3 position, float4 row0, float4 row1, float4 row2) {
    return float3(
        dot(row0.xyz, position) + row0.w,
        dot(row1.xyz, position) + row1.w,
        dot(row2.xyz, position) + row2.w
    );
}

float3 RotatePrevious(float3 direction, float4 row0, float4 row1, float4 row2) {
    // Sky is infinitely distant. Applying camera translation to its ray would
    // make walking smear the sky even though only camera rotation can move it.
    return float3(
        dot(row0.xyz, direction),
        dot(row1.xyz, direction),
        dot(row2.xyz, direction)
    );
}

float2 ClipMotionToViewport(float2 uv, float2 motion) {
    float2 low = 0.5f * ScreenData.zw;
    float2 high = 1.0f - low;
    float scale = 1.0f;
    if (motion.x > 0.0f) {
        scale = min(scale, (high.x - uv.x) / motion.x);
    } else if (motion.x < 0.0f) {
        scale = min(scale, (low.x - uv.x) / motion.x);
    }
    if (motion.y > 0.0f) {
        scale = min(scale, (high.y - uv.y) / motion.y);
    } else if (motion.y < 0.0f) {
        scale = min(scale, (low.y - uv.y) / motion.y);
    }
    return motion * saturate(scale);
}

float2 BoundedMotion(float2 uv, float2 previousUv, float layerStrength) {
    float2 motion = (previousUv - uv) * MotionOptions.x * layerStrength;
    float2 motionPixels = motion * ScreenData.xy;
    float speedPixels = length(motionPixels);
    float maxPixels = max(MotionOptions.y, 0.0f);
    if (speedPixels > maxPixels && speedPixels > 0.0001f) {
        motion *= maxPixels / speedPixels;
        speedPixels = maxPixels;
    }

    // A soft threshold avoids both subpixel shimmer and a visible on/off step
    // when camera motion crosses the configured minimum velocity.
    float threshold = max(MotionOptions.z, 0.0f);
    float activation = Smooth01(
        (speedPixels - threshold) / max(0.5f, threshold * 0.5f)
    );
    return ClipMotionToViewport(uv, motion * activation);
}

float DepthAcceptance(
    float2 sampleUv,
    bool centerFirstPerson,
    bool centerSky,
    float centerDepth,
    float motionPixels
) {
    float2 depthUv = DepthTexelCenter(sampleUv);
    bool firstPersonAvailable = LayerFlags.z > 0.5f;
    float firstPersonRaw = firstPersonAvailable
        ? tex2Dlod(FirstPersonDepth, float4(depthUv, 0.0f, 0.0f)).r
        : 0.0f;
    bool sampleFirstPerson = firstPersonAvailable && GeometryDepth(firstPersonRaw);
    if (sampleFirstPerson != centerFirstPerson) {
        return 0.0f;
    }

    float sampleRaw;
    float sampleDepth;
    if (centerFirstPerson) {
        sampleRaw = firstPersonRaw;
        sampleDepth = LinearDepth(sampleRaw, CurrentFirstPersonDepth.xy, LayerFlags.y > 0.5f);
    } else {
        sampleRaw = tex2Dlod(WorldDepth, float4(depthUv, 0.0f, 0.0f)).r;
        bool sampleSky = SkyDepth(sampleRaw, LayerFlags.x > 0.5f);
        if (sampleSky != centerSky || (!sampleSky && !GeometryDepth(sampleRaw))) {
            return 0.0f;
        }
        if (centerSky) {
            return 1.0f;
        }
        sampleDepth = LinearDepth(sampleRaw, CurrentWorldDepth.xy, LayerFlags.x > 0.5f);
    }

    // Relative depth is stable across the enormous FNV near/far range. The
    // tolerance grows modestly with blur length so grazing planes remain
    // continuous, while true foreground/background discontinuities are still
    // rejected before they can form bright silhouette halos.
    float relativeDifference = abs(sampleDepth - centerDepth)
        / max(min(sampleDepth, centerDepth), 0.01f);
    float tolerance = 0.025f + min(motionPixels * 0.002f, 0.075f);
    return 1.0f - Smooth01(
        (relativeDifference - tolerance) / max(tolerance, 0.0001f)
    );
}

float4 Main(float2 requestedUv : TEXCOORD0) : COLOR0 {
    float2 uv = DepthTexelCenter(requestedUv);
    float4 current = tex2Dlod(SceneColor, float4(uv, 0.0f, 0.0f));

    bool firstPersonAvailable = LayerFlags.z > 0.5f;
    float firstPersonRaw = firstPersonAvailable
        ? tex2Dlod(FirstPersonDepth, float4(uv, 0.0f, 0.0f)).r
        : 0.0f;
    bool centerFirstPerson = firstPersonAvailable && GeometryDepth(firstPersonRaw);

    float centerDepth = 0.0f;
    bool centerSky = false;
    float2 previousUv = uv;
    float layerStrength = 1.0f;

    if (centerFirstPerson) {
        // A missing previous first-person projection is not permission to use
        // the world transform for the weapon. Preserve it until its own camera
        // pair is valid.
        if (LayerFlags.w < 0.5f || MotionOptions.w <= 0.0f) {
            return current;
        }
        centerDepth = LinearDepth(
            firstPersonRaw,
            CurrentFirstPersonDepth.xy,
            LayerFlags.y > 0.5f
        );
        float3 currentPosition =
            ViewRay(uv, CurrentFirstPersonFrustum) * centerDepth;
        float3 previousPosition = TransformPrevious(
            currentPosition,
            FirstPersonRow0,
            FirstPersonRow1,
            FirstPersonRow2
        );
        if (previousPosition.z <= max(PreviousFirstPersonDepth.x, 0.001f)) {
            return current;
        }
        previousUv = ProjectPrevious(previousPosition, PreviousFirstPersonFrustum);
        layerStrength = MotionOptions.w;
    } else {
        float worldRaw = tex2Dlod(WorldDepth, float4(uv, 0.0f, 0.0f)).r;
        centerSky = SkyDepth(worldRaw, LayerFlags.x > 0.5f);
        if (!centerSky && !GeometryDepth(worldRaw)) {
            return current;
        }

        float3 previousPosition;
        if (centerSky) {
            previousPosition = RotatePrevious(
                ViewRay(uv, CurrentWorldFrustum),
                WorldRow0,
                WorldRow1,
                WorldRow2
            );
        } else {
            centerDepth = LinearDepth(
                worldRaw,
                CurrentWorldDepth.xy,
                LayerFlags.x > 0.5f
            );
            previousPosition = TransformPrevious(
                ViewRay(uv, CurrentWorldFrustum) * centerDepth,
                WorldRow0,
                WorldRow1,
                WorldRow2
            );
        }
        if (previousPosition.z <= max(PreviousWorldDepth.x, 0.001f)) {
            return current;
        }
        previousUv = ProjectPrevious(previousPosition, PreviousWorldFrustum);
    }

    previousUv = clamp(previousUv, 0.5f * ScreenData.zw, 1.0f - 0.5f * ScreenData.zw);
    float2 motion = BoundedMotion(uv, previousUv, layerStrength);
    float motionPixels = length(motion * ScreenData.xy);
    if (motionPixels <= 0.0001f) {
        return current;
    }

    // Trapezoidal endpoint weights integrate a uniform shutter interval
    // without over-emphasizing either the current or oldest sample.
    float3 sum = current.rgb * 0.5f;
    float weightSum = 0.5f;
    // The bound is a compile-time quality constant and therefore has exact
    // executed work. Keeping one uniform loop avoids multiplying this fairly
    // substantial depth/layer rejection body into hundreds of static ps_3_0
    // instructions, which is friendlier to D3D9-era instruction caches.
    [loop]
    for (int sampleIndex = 1; sampleIndex < MOTION_BLUR_SAMPLES; ++sampleIndex) {
        float t = (float)sampleIndex / (float)(MOTION_BLUR_SAMPLES - 1);
        float2 sampleUv = uv + motion * t;
        float endpointWeight =
            sampleIndex == (MOTION_BLUR_SAMPLES - 1) ? 0.5f : 1.0f;
        float weight = endpointWeight * DepthAcceptance(
            sampleUv,
            centerFirstPerson,
            centerSky,
            centerDepth,
            motionPixels
        );
        float3 sampleColor =
            tex2Dlod(SceneColor, float4(sampleUv, 0.0f, 0.0f)).rgb;
        sum += sampleColor * weight;
        weightSum += weight;
    }

    return float4(sum / max(weightSum, 0.0001f), current.a);
}
