...
PS_OUTPUT main(PS_INPUT IN) {
    ...
    #if defined(SPECULAR)
        float materialResponse = getSpecularGlossPower(glossPower);
    #else
        float materialResponse = getRoughness(normal.a);
    #endif

    #if defined(SPECULAR) && !defined(ONLY_SPECULAR)
        float nativeSpecularFade = saturate(IN.lightDir.w);
    #else
        float nativeSpecularFade = 1.0;
    #endif
    
    //if (TESR_DebugVar.y > 0.0) {
    //    OUT.color.a = 1;
    //    if (TESR_DebugVar.y > 0.1)
    //        OUT.color.rgb = materialResponse.xxx;
    //    else
    ...
    #else
        float3 lighting = getSunLighting(IN.lightDir.xyz, PSLightColor[0].rgb * shadowMultiplier, viewDir, normal.xyz, pbrSurface);
    #endif
    ...
    // Other light sources.
        ...
        #else
            lighting += getPointLightLighting(IN.light2Dir.xyz, IN.light2Dir.w, PSLightColor[1].rgb, viewDir, normal.xyz, pbrSurface);
        #endif
    #endif
        ...
        #else
            lighting += getPointLightLighting(IN.light3Dir.xyz, IN.light3Dir.w, PSLightColor[2].rgb, viewDir, normal.xyz, pbrSurface);
        #endif
    #endif
    ...
    #ifndef NO_FOG
        #ifdef ONLY_LIGHT
            finalColor.rgb *= 1.0 - saturate(IN.fogColor.a);
        #else
            #ifndef OPT
...
PS_OUTPUT main(PS_INPUT IN) {
    ...
    #if defined(SPECULAR)
        float materialResponse = getSpecularGlossPower(glossPow);
        float nativeSpecularFade = saturate(IN.lPosition.w);
    #else
        float materialResponse = getRoughness(normal.a);
        float nativeSpecularFade = 1.0;
    #endif
