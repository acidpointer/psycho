...
float2 WeightedTap(float2 uv, float centerDepth, float weight) {
    ...
    float sampleDepth = sampleValue.g * ContactDepthKeyRange;
...
float4 Main(PixelInput input) : COLOR0 {
    ...
    float centerDepth = center.g * ContactDepthKeyRange;
    float2 sum = float2(center.r * 0.40f, 0.40f);
    sum += WeightedTap(input.uv + float2(ScreenData.z, 0.0f), centerDepth, 0.15f);
    sum += WeightedTap(input.uv - float2(ScreenData.z, 0.0f), centerDepth, 0.15f);
    sum += WeightedTap(input.uv + float2(0.0f, ScreenData.w), centerDepth, 0.15f);
    sum += WeightedTap(input.uv - float2(0.0f, ScreenData.w), centerDepth, 0.15f);
