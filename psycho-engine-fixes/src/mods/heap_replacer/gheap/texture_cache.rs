...
type DeadSet = RwLock<HashSet<usize, FxBuildHasher>>;

static TEXTURE_DEAD_SET: std::sync::LazyLock<DeadSet> =
    std::sync::LazyLock::new(|| RwLock::new(HashSet::with_hasher(FxBuildHasher)));
...
pub unsafe extern "fastcall" fn hook_nisourcetexture_dtor(this: *mut c_void) {
    TEXTURE_DEAD_SET.write().insert(this as usize);
...
unsafe fn chain_has_dead_entry(mut entry: *const u32) -> bool {
    let dead_set = TEXTURE_DEAD_SET.read();
    ...
    unsafe {
        loop {
            ...
            if !value_ptr.is_null() {
                ...
                if inner_ptr != 0 && dead_set.contains(&inner_ptr) {
                    return true;
                }
            }
