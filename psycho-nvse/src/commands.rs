...
use crate::mods::heap_replacer::gheap::{allocator, block, pool};
...
nvse_command!(PsychoQuarantine, cmd, {
    ...
    cmd.print("=== Heap Status ===");
    cmd.print(&format!("Pool committed:      {:.1} MB", pool_mb));
    cmd.print(&format!("Pool live cells:     {}", pool::live_cells()));
    cmd.print(&format!("Block committed:     {:.1} MB ({} blocks)", block_mb, block::block_count()));
...
nvse_command!(PsychoCellUnload, cmd, {
    ...
    cmd.print(&format!("Current: commit={}MB, pool={}MB", commit_mb, pool_mb));
    cmd.print(&format!(
